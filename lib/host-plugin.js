...
import { CallId, LlmAdapter, LlmError, MessageId, createMessage, freezeMessage } from "@deepseek-ai/dsh-llm";
...
var CodexDshAdapter = class extends LlmAdapter {
	...
	async *stream(options) {
		...
		state.location = {
			turn: agent.session.events.findLast((event) => event.type === "turn/start")?.data.turn ?? 1,
	...
	appendActivity(agent, threadId, turnId, item, phase, state) {
		...
		if (!state.startedActivities.has(id)) {
			...
			const callId = CallId(`relay-codex:${JSON.stringify([
				threadId,
				turnId,
				id
			])}`);
