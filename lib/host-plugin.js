...
var CodexSessionRuntime = class extends EventEmitter {
	...
	async interruptTurn(threadId, turnId) {
		const activeCommands = (this.requireSession(threadId).turns.find((candidate) => candidate.id === turnId)?.items ?? []).filter((item) => item?.type === "commandExecution" && item.status === "inProgress");
		...
		const failures = [];
		if (itemIds.size > 0) try {
			for (const terminal of await this.listBackgroundTerminals(threadId)) if (itemIds.has(terminal.itemId) && terminal.processId) processIds.add(String(terminal.processId));
		...
		if (itemIds.size > 0) try {
			...
			for (const terminal of afterInterrupt) if (itemIds.has(terminal.itemId) && terminal.processId) remainingProcessIds.add(String(terminal.processId));
			await this.terminateBackgroundProcesses(threadId, remainingProcessIds);
			const remaining = (await this.listBackgroundTerminals(threadId)).filter((terminal) => itemIds.has(terminal.itemId));
...
async function importCodexGeneratedImage(item, roots, attachments) {
	...
	const encoded = result.match(/^data:(image\/(?:png|jpeg|webp|gif));base64,(.+)$/s)?.[2] ?? result;
...
var CodexDshAdapter = class extends LlmAdapter {
	...
	async projectActivity(agent, threadId, turnId, message, state) {
		...
		if (message.method === "item/commandExecution/outputDelta") {
			...
			return commandOutputDelta(state, state.commandKeys.get(params.itemId) ?? commandItemKey(params.itemId), "native", params.delta ?? "");
		}
...
function completeCommandOutput(state, id, aggregatedOutput) {
	...
	} else if (completeText && completeText !== block.text && !sources?.raw) block.text = completeText;
	block.closed = true;
	chunks.push({
		type: "block-end",
		index: block.index,
...
function commandOutputDelta(state, id, source, delta) {
	...
	const duplicateLength = commonLength === delta.length || commonLength === comparable.length ? commonLength : 0;
