...
function resolveCodexLaunch({ command, env = process.env, execPath = process.execPath, platform = process.platform, arch = process.arch, homeDirectory = homedir(), candidatePaths, fsApi = {
	accessSync,
	realpathSync,
	statSync
}, resolvePackage = require.resolve } = {}) {
	const configured = nonBlank$1(command);
	if (configured !== void 0) return resolveRequested(configured, "config");
	const environmentCommand = nonBlank$1(env.RELAY_CODEX_COMMAND);
	if (environmentCommand !== void 0) return resolveRequested(environmentCommand, "environment");
	return resolveAuto({
		env,
		execPath,
		platform,
	...
	function resolveRequested(value, origin) {
		if (value.toLowerCase() === "auto") return resolveAuto({
			env,
			execPath,
			platform,
			arch,
		...
		if (value.toLowerCase() === "bundled") return bundledLaunch({
			platform,
			arch,
			execPath,
			resolvePackage
		...
		return Object.freeze({
			command: validateExecutablePath(value, {
				platform,
				fsApi
			}),
			argsPrefix: [],
...
function resolveAuto({ env, execPath, platform, arch, homeDirectory, candidatePaths, fsApi, resolvePackage }) {
	const failures = [];
	for (const candidate of candidatePaths ?? defaultCandidatePaths({
		...
		const path = nonBlank$1(candidate);
		if (path === void 0) continue;
		try {
			const command = validateExecutablePath(path, {
				platform,
				fsApi
			});
			return Object.freeze({
			...
			failures.push({
				path,
				code: error.code ?? "CODEX_EXECUTABLE_INVALID"
			});
		}
	}
	const launch = bundledLaunch({
		platform,
		arch,
		execPath,
		resolvePackage
...
function defaultCandidatePaths({ env, platform, homeDirectory }) {
	const candidates = [];
	if (platform === "darwin") for (const base of ["/Applications", join(homeDirectory, "Applications")]) candidates.push(join(base, "ChatGPT.app", "Contents", "Resources", "codex"), join(base, "Codex.app", "Contents", "Resources", "codex"));
		...
		for (const base of bases) candidates.push(join(base, "ChatGPT", "resources", "codex.exe"), join(base, "Codex", "resources", "codex.exe"));
	} else if (platform === "linux") for (const base of [env.XDG_DATA_HOME && join(env.XDG_DATA_HOME, "applications"), join(homeDirectory, ".local", "share", "applications")].filter(Boolean)) candidates.push(join(base, "ChatGPT", "resources", "codex"), join(base, "Codex", "resources", "codex"));
	const pathSeparator = platform === "win32" ? ";" : delimiter;
	const pathEntries = typeof env.PATH === "string" ? env.PATH.split(pathSeparator) : [];
	for (const entry of pathEntries) {
		if (!nonBlank$1(entry)) continue;
...
function validateExecutablePath(value, { platform = process.platform, fsApi = {
	accessSync,
	realpathSync,
	statSync
} } = {}) {
	const path = nonBlank$1(value);
	if (path === void 0 || !isAbsoluteForPlatform(path, platform)) throw executableInvalidError(value, "an absolute executable path is required");
	...
	try {
		if (!stat(canonical).isFile()) throw executableInvalidError(path, "the path is not a regular file");
...
function codexSpawnError(error, command, source) {
	...
	wrapped.message = `Unable to start Codex from ${JSON.stringify(command)} (${source}). Set RELAY_CODEX_COMMAND to an absolute Codex executable path, or reinstall relay-dsh-plugin-codex to restore its bundled runtime.`;
	wrapped.cause = error;
	return wrapped;
}
function executableNotFoundError(path) {
	const error = /* @__PURE__ */ new Error(`Codex executable was not found at ${JSON.stringify(path)}. Set RELAY_CODEX_COMMAND to an absolute executable path, or use auto/bundled.`);
	error.code = "CODEX_EXECUTABLE_NOT_FOUND";
	error.path = path;
	return error;
}
function executableInvalidError(path, reason, cause) {
	const error = /* @__PURE__ */ new Error(`Codex executable at ${JSON.stringify(path)} is invalid: ${reason}.`);
...
var CodexAppServerClient = class extends EventEmitter {
	constructor({ command, args = RELAY_CODEX_APP_SERVER_ARGS, requestTimeoutMs = 3e4, clientInfo = NATIVE_CODEX_CLIENT_INFO, capabilities = NATIVE_CODEX_CAPABILITIES, launchOptions = {} } = {}) {
		super();
		this.launch = resolveCodexLaunch({
			command,
			...launchOptions
		});
	...
	async start() {
		...
		try {
			...
			this.emit("diagnostic", `Automatic Codex runtime at ${this.launch.command} failed preflight: ${error?.code ?? "ERROR"} ${error?.message ?? error}; falling back to bundled runtime.`);
	...
	async stopProcess() {
		...
		this.process = null;
		this.failAll(/* @__PURE__ */ new Error("codex app-server process stopped"));
		child.kill("SIGTERM");
...
function assertUsableModelList(result) {
	if (!Array.isArray(result?.data)) {
		const error = /* @__PURE__ */ new Error("Codex App Server returned an invalid model/list response.");
		error.code = "CODEX_MODEL_LIST_INVALID";
		throw error;
	}
	if (!result.data.some((model) => typeof model?.id === "string" && model.id.trim())) {
		const error = /* @__PURE__ */ new Error("Codex App Server returned no usable models.");
...
function connectedCodexConnectionStatus(now = Date.now(), details = {}) {
	return Object.freeze({
		state: "connected",
...
function codexConnectionFailure(error, now = Date.now()) {
	...
	if (code === "CODEX_EXECUTABLE_INVALID") return failure("unavailable", code, "Codex could not start because the configured executable path is invalid.", "Set codexCommand or RELAY_CODEX_COMMAND to an absolute executable file, or use auto/bundled.", now);
	...
	if (["CODEX_MODEL_LIST_INVALID", "CODEX_MODEL_LIST_EMPTY"].includes(code)) return failure("connection-failed", code, "Codex App Server did not return a usable model list.", "Restart DSH and verify the selected Codex runtime and authentication.", now);
...
var CodexSessionRuntime = class CodexSessionRuntime extends EventEmitter {
	...
	async initialize() {
		...
		try {
			...
			const [modelsResult, accountResult, threadsResult] = await Promise.all([
				this.client.initialModels ?? this.client.request("model/list", {
					limit: 50,
					includeHidden: false
				}),
			...
			this.setConnectionStatus(connectedCodexConnectionStatus(Date.now(), { runtime: this.client.runtimeInfo ?? null }));
			this.emitChange();
	...
	isReloadSafe() {
		...
		return [...this.sessions.values()].every((session) => session.turns.every((turn) => turn.status !== "inProgress"));
	}
	adoptStateFrom(previous) {
		if (!(previous instanceof CodexSessionRuntime)) throw new TypeError("Codex runtime state must come from another CodexSessionRuntime");
		if (!previous.isReloadSafe()) throw new Error("Codex runtime cannot adopt state while a turn or interaction is active");
...
var CodexRuntimeController = class extends EventEmitter {
	constructor(config = {}) {
		super();
		this.config = { ...config };
	...
	async start(command) {
		...
		if (!this.closed && this.desiredCommand !== command) this.reload(this.desiredCommand).catch((error) => this.logReloadFailure(error));
		return entry.ready;
	}
	async reload(command) {
		...
		this.reloadPromise = this.processReload().then((result) => {
			...
			this.reloadPromise = null;
			if (succeeded && !this.closed && this.desiredCommand !== this.current?.command) this.reload(this.desiredCommand).catch((error) => this.logReloadFailure(error));
		});
	...
	async processReload() {
		...
		try {
			...
			this.onReload?.({
				command,
				models: candidate.runtime.models
			});
	...
	createClient(command) {
		...
		if (typeof this.config.clientFactory === "function") return this.config.clientFactory(command);
	...
	logReloadFailure(error) {
		this.current?.runtime.addDiagnostic?.(`Codex runtime reload failed: ${error?.message ?? error}`);
		this.logger.error?.(`Relay Codex runtime reload failed: ${error?.stack ?? error}`);
		this.emit("change");
	}
	whenReady() {
		return this.ready ?? Promise.reject(notStartedError());
	}
	status() {
		return this.current?.runtime.status() ?? initialUnavailableStatus();
	}
	listModels() {
		return structuredClone(this.current?.runtime.models ?? []);
	}
	hasSession(sessionId) {
		return this.current?.runtime.sessions.has(sessionId) ?? false;
	}
	getSession(...args) {
		return this.current?.runtime.getSession(...args) ?? null;
	}
	...
	listWorkspaceThreads(...args) {
		return this.current.runtime.listWorkspaceThreads(...args);
	}
	readThread(...args) {
		return this.current.runtime.readThread(...args);
	}
	createSession(...args) {
		return this.current.runtime.createSession(...args);
	}
	forkSession(...args) {
		return this.current.runtime.forkSession(...args);
	}
	resumeSession(...args) {
		return this.current.runtime.resumeSession(...args);
	}
	sendMessage(...args) {
		return this.current.runtime.sendMessage(...args);
	}
	interruptTurn(...args) {
		return this.current.runtime.interruptTurn(...args);
	}
	releaseSession(...args) {
		return this.current.runtime.releaseSession(...args);
	}
	resolveRequest(...args) {
		return this.current.runtime.resolveRequest(...args);
	}
	respondDynamicTool(...args) {
		return this.current.runtime.respondDynamicTool(...args);
	}
	rejectRequest(...args) {
		return this.current.runtime.rejectRequest(...args);
	}
	request(...args) {
		return this.current.client.request(...args);
	}
	subscribeActivity(listener) {
		return subscribe(this, "activity", listener);
	}
	subscribeRequest(listener) {
		return subscribe(this, "request", listener);
	}
	subscribeStatus(listener) {
		return subscribe(this, "connectionStatus", listener);
	}
	subscribeNotification(listener) {
		return subscribe(this, "notification", listener);
	}
};
...
function notStartedError() {
	const error = /* @__PURE__ */ new Error("Codex runtime has not started");
...
function initialUnavailableStatus() {
	return {
		state: "unavailable",
		code: "CODEX_RUNTIME_NOT_STARTED",
...
const DSH_MCP_TOOL_ALIAS_PREFIX = "relay_mcp__";
...
function codexDshToolSurface(dshTools = [], builtins = CODEX_APP_DYNAMIC_TOOLS) {
	const bindings = codexDshToolBindings(dshTools);
	const tools = dshTools.map((tool) => ({
		type: "function",
		name: bindings.originalToAlias.get(String(tool.name)),
		description: tool.description,
		inputSchema: structuredClone(tool.parameters)
	}));
	return {
		dynamicTools: tools.length === 0 ? structuredClone(builtins) : [...structuredClone(builtins), {
			type: "namespace",
			name: "dsh",
			description: "Tools contributed to this conversation through the DSH plugin runtime.",
			tools
		}],
		aliasToOriginal: bindings.aliasToOriginal
	};
}
function codexDshToolBindings(dshTools = []) {
	const aliasToOriginal = /* @__PURE__ */ new Map();
	const originalToAlias = /* @__PURE__ */ new Map();
	const usedAliases = /* @__PURE__ */ new Set();
	for (const tool of dshTools) {
		...
		if (!original.startsWith("mcp__")) {
			...
			usedAliases.add(original);
		}
	}
	for (const tool of dshTools) {
		const original = String(tool.name);
		if (originalToAlias.has(original)) continue;
		const base = `${DSH_MCP_TOOL_ALIAS_PREFIX}${original.slice(5)}`;
		let alias = base;
		if (usedAliases.has(alias)) {
			const digest = createHash("sha256").update(original).digest("hex").slice(0, 10);
			alias = `${base}__${digest}`;
			let suffix = 2;
			while (usedAliases.has(alias)) alias = `${base}__${digest}_${suffix++}`;
		}
...
async function handleDynamicTool(runtime, request, adapter, agent, sessionId) {
	...
	if (namespace === "dsh") {
		const originalTool = adapter.dshToolName(sessionId, tool);
		if (!originalTool) {
...
var CodexDshAdapter = class extends LlmAdapter {
	constructor({ runtime, ready, linkStore = null, attachments = null, logger = console, dynamicTools = CODEX_APP_DYNAMIC_TOOLS, executionGuidance = true, executionMode = "enhanced" }) {
		super();
	...
	hasDshTool(sessionId, name) {
		return this.dshToolName(sessionId, name) !== void 0;
	}
	...
	async *stream(options) {
		...
		const dshToolSurface = codexDshToolSurface(this.executionMode === "native" ? [] : options.tools ?? [], this.dynamicTools);
		const threadId = await this.ensureThread(sessionId, dshToolSurface.dynamicTools, inheritedCodexProvenance(options.messages));
...
const CODEX_SETTINGS_SCHEMA = z.object({ codexCommand: z.string().default("") });
async function apply(ctx, config = {}) {
	...
	try {
		...
		ctx.get("settings")?.installSection(ctx, CODEX_SETTINGS_NAMESPACE, CODEX_SETTINGS_SCHEMA, base, {
			...
			onChange() {
				if (controller.started) controller.reload(source().codexCommand).catch((error) => {
...
//#endregion
export { CODEX_SETTINGS_NAMESPACE, CODEX_SETTINGS_SCHEMA, apply, inject, name };
